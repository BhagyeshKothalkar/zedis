...
static void test_hash_table(void) {
  ...
  ASSERT(zedis_ht_get(&ht, "foo", 3, val, sizeof(val), &len) != 0,
         "ht miss after del");
...
static void test_btree(void) {
  ...
  char reply[4096];
  int rlen = 0;
  ASSERT(zedis_btree_range(&tree, 0, -1, reply, sizeof(reply), &rlen) == 0,
         "btree range");
  ...
  ASSERT(zedis_btree_score(&tree, "alpha", 5, &score) == 0,
         "btree score alpha");
...
static void test_append_log(void) {
  ...
  char reply[4096];
  int rlen = 0;
  ASSERT(zedis_aol_lrange(&log, &meta, 0, -1, reply, sizeof(reply), &rlen) == 0,
         "lrange");
...
static void test_ring_buffer(void) {
  ...
  ASSERT(slot.payload_len == 4 && memcmp(slot.payload, "msg1", 4) == 0,
         "ring payload");
...
static void test_key_registry(void) {
  ...
  int64_t score = 0;
  ASSERT(zedis_zset_reg_score(&zsets, "z", 1, "a", 1, &score) == 0,
         "zscore via reg");
  ...
  ASSERT(zedis_list_reg_lpush(&lists, &log, "l", 1, "x", 1) == 0,
         "lpush via reg");
