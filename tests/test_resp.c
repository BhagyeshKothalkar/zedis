...
static void test_parser_basics(void) {
    ...
    ASSERT(val.bulk_len == 4 && memcmp(val.bulk, "PING", 4) == 0, "simple content");
    ...
    ASSERT(val.bulk_len == 10 && memcmp(val.bulk, "ERR custom", 10) == 0, "error content");
    resp_value_release(&parser, &val);
...
static void test_parser_array(void) {
    ...
    ASSERT(n1->bulk_len == 3 && memcmp(n1->bulk, "GET", 3) == 0, "node 1 content");

    resp_node_t *n2 = n1->next;
    ASSERT(n2 != NULL, "node 2 exists");
    ASSERT(n2->type == RESP_TYPE_BULK, "node 2 type");
    ASSERT(n2->bulk_len == 4 && memcmp(n2->bulk, "keys", 4) == 0, "node 2 content");
...
static void test_parser_fragmented(void) {
    ...
    ASSERT(resp_parser_next(&parser, &val) == RESP_NEED_MORE, "parse frag 1 needs more");

    ASSERT(resp_parser_feed(&parser, "NG\r", 3) == 0, "feed frag 2");
    ASSERT(resp_parser_next(&parser, &val) == RESP_NEED_MORE, "parse frag 2 needs more");
    ...
    ASSERT(val.bulk_len == 4 && memcmp(val.bulk, "PING", 4) == 0, "fragmented simple content");
